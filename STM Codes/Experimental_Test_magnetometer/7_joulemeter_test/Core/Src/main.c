...
int main(void)
{
  ...
  while (1)
  {
    ...
    // 2. Marker blink DISABLED -- confirmed via joulemeter (with vs.
    //    without) that the LED wasn't the dominant contributor to the
    //    active window's current anyway (MCU-active + SX1280 tail
    //    dominate at the mA level, dwarfing both the LED and the
    //    magnetometer's own tens-of-uA read current). The diagnostic
    //    200ms quiet gap that was here has also been removed -- it was
    //    ~half of the entire active window, added only to make TX and the
    //    mag read visually separable on a 100ms-resolution trace. Since
    //    the active window (however brief) turns out to dominate total
    //    average current far more than the Stop-mode floor does, that gap
    //    was pure waste in a real measurement -- gone now that we don't
    //    need it for trace-reading anymore. Re-enable only for another
    //    diagnostic run (100ms LED on/off around the magnetometer read
    //    below), not for a real power number.
    ...
    // 3. Shut down peripherals, flash LED, and enter Stop Mode (wakes up in 10s)
    STM32_EnterStopMode();
    /* USER CODE END WHILE */
