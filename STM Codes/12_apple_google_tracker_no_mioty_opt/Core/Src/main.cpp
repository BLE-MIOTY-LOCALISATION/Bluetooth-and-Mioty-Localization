 ...
 *  Configured for: BLE MIOTY TRACKER III
 * ============================================================ */
static const uint8_t fmdn_eid[20] = {
    0x14, 0xDD, 0x53, 0x86, 0xC0, 0x69, 0x70, 0xC5, 0x3B, 0x2A,
    0xC0, 0xF6, 0xC2, 0x6A, 0x90, 0x50, 0x43, 0xB3, 0x63, 0xF5
};
...
void LED_Boot_Sequence(void)
{
    ...
    LED_Blink(1, 500, 200);  // 1 long blink: BLE init done
}
...
// Send arbitrary ADV_NONCONN_IND payload on one BLE channel.
uint8_t SX1280_SendOnChannel(uint8_t freq0, uint8_t freq1, uint8_t freq2, uint8_t white_seed, uint8_t *payload, uint8_t len)
{
    // Set RF frequency
    ...
    // Write PDU to TX buffer at offset 0x80
    SX1280_WriteBuffer(0x80, payload, len);
...
// Transmit one Google FMDN BLE beacon across all 3 advertising channels.
void SX1280_SendGoogleBeacon(void)
{
    uint8_t adv_data[29] = {
        0x02, 0x01, 0x06,
    ...
    for (int i = 0; i < 20; i++) adv_data[8 + i] = fmdn_eid[i];

    // Tracker III Unique Google MAC (CC:BB:CC:DD:EE:FF)
    uint8_t mac[6] = {0xCC, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
    ...
    SX1280_SendOnChannel(0xB8, 0xC4, 0xEC, 0x53, payload, 37); // CH37
    HAL_Delay(5);
    SX1280_SendOnChannel(0xBA, 0x9D, 0x89, 0x33, payload, 37); // CH38
    HAL_Delay(5);
    SX1280_SendOnChannel(0xBE, 0xC4, 0x55, 0x73, payload, 37); // CH39
}

// Transmit one Apple Find My Offline Finding beacon across all 3 advertising channels.
void SX1280_SendAppleBeacon(void)
{
    // Apple Offline Finding Payload structure (31 bytes total)
    // First byte is AD Length: 0x1E (30 bytes follow)
    // Generated specifically for BLE MIOTY TRACKER III
    uint8_t adv_data[31] = {
        0x1E, 0xFF, 0x4C, 0x00, 0x12, 0x19, 0x00, 
        // 22 bytes of pubkey payload (pub_x[6:28])
        0x47, 0x2F, 0xAC, 0x9C, 0x69, 0x10, 0x3A, 0xF7, 0x12, 0x26, 0xA8,
        0x0A, 0xCE, 0xCB, 0xE7, 0x67, 0xE4, 0x1C, 0x40, 0x8E, 0x41, 0x3E,
        0x01, // first two bits of pub_x[0] (0x72 >> 6 = 1)
        0x00  // hint byte
    };

    // Bound Apple MAC Address for Tracker III (LSB-first on-air):
    // Public key bytes[0..5]: 0x72, 0x21, 0x76, 0x33, 0x11, 0xE5
    // MSB (mac[5]) has 0xC0 bitmask: 0x72 | 0xC0 = 0xF2
    uint8_t mac[6] = {0xE5, 0x11, 0x33, 0x76, 0x21, 0xF2};

    uint8_t payload[39];
    payload[0] = 0x42;
    payload[1] = 6 + 31; // 6 byte MAC + 31 bytes of adv_data = 37 bytes PDU
    for (int i = 0; i < 6; i++)  payload[2 + i] = mac[i];
    for (int i = 0; i < 31; i++) payload[8 + i] = adv_data[i];

    SX1280_SendOnChannel(0xB8, 0xC4, 0xEC, 0x53, payload, 39); // CH37
    HAL_Delay(5);
    SX1280_SendOnChannel(0xBA, 0x9D, 0x89, 0x33, payload, 39); // CH38
    HAL_Delay(5);
    SX1280_SendOnChannel(0xBE, 0xC4, 0x55, 0x73, payload, 39); // CH39
}
...
void STM32_EnterStopMode(uint32_t seconds)
{
    ...
    // 2. Disable SPI1 peripheral and clamp SPI bus pins to pull-down
...
int main(void)
{
    ...
    while (1)
    {
        ...
        debug_stage = 21;
        SX1280_SendGoogleBeacon();
        
        HAL_Delay(10); // Small gap between beacons
        
        debug_stage = 22;
        SX1280_SendAppleBeacon();

        // Optimized Heartbeat: 5ms pulse every 2nd cycle (every ~6 seconds)
        ...
        /* --- Sleep for 3 seconds in Stop Mode --- */
        debug_stage = 40;
        STM32_EnterStopMode(3);
    }
}
