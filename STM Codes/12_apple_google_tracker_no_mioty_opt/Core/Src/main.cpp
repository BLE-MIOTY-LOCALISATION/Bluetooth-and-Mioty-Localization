...
static const uint8_t fmdn_eid[20] = {
    0x0C, 0x98, 0x1D, 0xBB, 0xF5, 0xDD, 0xE0, 0x1E, 0x6E, 0x04,
    0xF1, 0x06, 0x68, 0xDB, 0xF3, 0xF6, 0x8A, 0xBC, 0x2F, 0xCF
};
...
void SX1280_SendGoogleBeacon(void)
{
    ...
    // Tracker II Unique Google MAC (distinct from Tracker I)
    uint8_t mac[6] = {0xBC, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
...
void SX1280_SendAppleBeacon(void)
{
    ...
    // Generated specifically for BLE MIOTY TRACKER II
    uint8_t adv_data[31] = {
        0x1E, 0xFF, 0x4C, 0x00, 0x12, 0x19, 0x00, 
        // 22 bytes of pubkey payload (pub_x[6:28])
        0xEC, 0x6B, 0xD2, 0x04, 0x28, 0x1E, 0x89, 0xB0, 0x93, 0x8E, 0x0F,
        0x65, 0x37, 0xFF, 0x73, 0x82, 0xD0, 0x84, 0x30, 0x06, 0xB4, 0x31,
        0x03, // first two bits of pub_x[0] (0xEB >> 6 = 3)
        0x00  // hint byte
    };

    // Bound Apple MAC Address for Tracker II (LSB-first on-air):
    // Public key bytes[0..5]: 0xEB, 0xE1, 0x97, 0x3D, 0x73, 0xAA
    // MSB (mac[5]) has 0xC0 bitmask: 0xEB | 0xC0 = 0xEB
    uint8_t mac[6] = {0xAA, 0x73, 0x3D, 0x97, 0xE1, 0xEB};
