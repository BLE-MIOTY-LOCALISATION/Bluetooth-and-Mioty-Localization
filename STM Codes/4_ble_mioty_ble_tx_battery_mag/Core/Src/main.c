...
uint8_t IIS2MDC_CalibrateNorthReference(void);
uint8_t IIS2MDC_GetOrientationBlinkCount(void);
...
// -----------------------------------------------------------------------
// IIS2MDC helper: read a single register over I2C2
// -----------------------------------------------------------------------
uint8_t IIS2MDC_ReadRegister(uint8_t reg, uint8_t *value) {
  if (HAL_I2C_Mem_Read(&hi2c2, IIS2MDC_I2C_ADDR, reg,
...
uint8_t IIS2MDC_WriteRegister(uint8_t reg, uint8_t value) {
  if (HAL_I2C_Mem_Write(&hi2c2, IIS2MDC_I2C_ADDR, reg,
                        I2C_MEMADD_SIZE_8BIT, &value, 1, 100) == HAL_OK) {
    return 1;
  }
  return 0;
...
// -----------------------------------------------------------------------
// Take one low-power single-shot magnetic sample, then return to idle.
// Orientation is compared against the boot-time reference direction.
// Returns blink count: 1=same direction, 2=opposite, 3=sideways/unclear,
// 8=read/config error.
// -----------------------------------------------------------------------
uint8_t IIS2MDC_ReadXYZ(int16_t *x, int16_t *y, int16_t *z) {
  uint8_t status = 0;
  ...
  if (!IIS2MDC_WriteRegister(IIS2MDC_CFG_REG_C, IIS2MDC_CFG_BDU)) {
    return 0;
  }
  if (!IIS2MDC_WriteRegister(IIS2MDC_CFG_REG_A, IIS2MDC_CFG_SINGLE_LP)) {
    return 0;
  }

  for (uint8_t i = 0; i < 20; i++) {
    HAL_Delay(2);
    if (!IIS2MDC_ReadRegister(IIS2MDC_STATUS_REG, &status)) {
      return 0;
    }
    if ((status & IIS2MDC_STATUS_ZYXDA) != 0) {
      break;
    }
  }
  if ((status & IIS2MDC_STATUS_ZYXDA) == 0) {
...
uint8_t IIS2MDC_GetOrientationBlinkCount(void) {
  ...
  int64_t dot = (int64_t)x * mag_ref_x + (int64_t)y * mag_ref_y;
  int64_t ref_norm = (int64_t)mag_ref_x * mag_ref_x + (int64_t)mag_ref_y * mag_ref_y;
  int64_t cur_norm = (int64_t)x * x + (int64_t)y * y;
  int64_t dot_abs = (dot < 0) ? -dot : dot;

  if (ref_norm == 0 || cur_norm == 0) {
    return 8;
  }

  // Same/opposite if within roughly 45 degrees of the reference/opposite vector.
  if ((dot_abs * dot_abs * 2) >= (ref_norm * cur_norm)) {
    return (dot >= 0) ? 1 : 2;
  }
  return 3;
}
...
// -----------------------------------------------------------------------
// Verify IIS2MDC magnetometer presence.
// WHO_AM_I register 0x4F should read 0x40.
// -----------------------------------------------------------------------
uint8_t IIS2MDC_CheckWhoAmI(void) {
  uint8_t whoami = 0;
  if (!IIS2MDC_ReadRegister(IIS2MDC_WHO_AM_I_REG, &whoami)) {
    return 0;
  }
...
void STM32_EnterStopMode(void) {
  ...
  //222222222222222 /* --- COMMENT OUT POST-SLEEP RECOVERY & BLE RE-CONFIG --- */
...
int main(void)
{
  ...
  // Expected radio sequence: 9 blinks, 6 blinks, 3 blinks, 1 blink
  SX1280_VerifyInit();

  // Magnetometer presence + reference direction check over I2C2.
  // Keep the board pointed in your chosen "north" direction during boot.
  // Good startup sequence: SX1280 alive blink, then 9,6,3,1,2.
  // If the final group is 8 blinks, IIS2MDC WHO_AM_I or reference read failed.
  uint8_t mag_ok = IIS2MDC_CheckWhoAmI() && IIS2MDC_CalibrateNorthReference();
  ...
  while (1)
  {
    // 1. Take one magnetometer sample and blink orientation sector.
    //    1=same as boot reference, 2=opposite, 3=sideways/unclear, 8=read error.
    LED_Blink(IIS2MDC_GetOrientationBlinkCount());

    // 2. Transmit BLE beacon on all 3 channels
    SX1280_SendBLEBeacon();

    // 3. Shut down peripherals and enter Stop Mode (wakes up in 1s)
    STM32_EnterStopMode();
    /* USER CODE END WHILE */
