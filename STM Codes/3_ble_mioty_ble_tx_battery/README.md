# BLE + mioty Module: Battery BLE Beacon Baseline

This STM32CubeIDE project is the non-magnetometer low-power BLE transmit baseline for the BLE + mioty localization module.

The firmware wakes periodically, transmits one BLE iBeacon advertisement set on the three BLE advertising channels, puts the SX1280 radio to sleep, enters STM32 Stop mode, and wakes again from the RTC alarm.

Project name:

```text
ble_mioty_ble_tx_battery
```

CubeMX file:

```text
ble_mioty_ble_tx_battery.ioc
```

This is the stable battery-focused baseline. Magnetometer experiments belong in project 4.
...
```text
ble_mioty_ble_tx_battery.launch
```

Expected build artifact:

```text
Debug/ble_mioty_ble_tx_battery.elf
```
...
| `ble_mioty_ble_tx_battery.ioc` | CubeMX project configuration |
| `ble_mioty_ble_tx_battery.launch` | CubeIDE debug/flash launch configuration |

## Difference From Project 4

Project 3 is intentionally BLE-only and does not include the IIS2MDC magnetometer.
...
Keep project 3 as the clean default low-power BLE baseline.
